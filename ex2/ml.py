...
def plotDecisionBoundary(theta, X, y):
    ...
    if X.shape[1] <= 3:
        ...
        z = [
            np.array(
                [mapFeature2(u[i], v[j]).dot(theta) for i in range(len(u))])
            for j in range(len(v))
        ]
        plt.plot(0, 0, 'g-', label='Decision boundary')
        plt.contour(u, v, z, colors='g', levels=[0.0])
...
def mapFeature(X, degree=6):
    ...
    """
    quads = Series([
        X.iloc[0]**(i - j) * X.iloc[1]**j for i in range(1, degree + 1)
        for j in range(i + 1)
    ])
...
def mapFeature2(X1, X2, degree=6):
    ...
    """
    quads = Series([
        X1**(i - j) * X2**j for i in range(1, degree + 1)
        for j in range(i + 1)
    ])
